...
BEGIN
    ...
    WITH stats AS (
        SELECT
            ...
            COUNT(*) FILTER (WHERE a.overall_score IS NOT NULL)::BIGINT AS completed,
            AVG(a.overall_score::FLOAT)                                  AS avg_sc,
            PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY a.overall_score) AS median_sc,
            MIN(a.overall_score)                                         AS min_sc,
            MAX(a.overall_score)                                         AS max_sc,
            AVG(a.confidence_level::FLOAT)                               AS avg_conf,
            AVG(a.technical_feasibility::FLOAT)                          AS avg_tf,
            AVG(a.economic_viability::FLOAT)                             AS avg_ev,
            AVG(a.circularity_potential::FLOAT)                          AS avg_cp,
            AVG(a.parameter_consistency_score::FLOAT)                    AS avg_pcs,
            AVG(a.r_strategy_alignment_score::FLOAT)                     AS avg_ras
        FROM public.user_assessments a
    ...
    by_industry AS (
        ...
        FROM (
            SELECT
                COALESCE(a.industry, 'uncategorized')       AS industry_key,
                COUNT(*)::INT                               AS cnt,
                ROUND(AVG(a.overall_score)::NUMERIC, 1)     AS ind_avg
            FROM public.user_assessments a
            WHERE (user_uuid IS NULL OR a.user_id = user_uuid)
              AND a.overall_score IS NOT NULL
            GROUP BY a.industry
    ...
    by_risk AS (
        SELECT jsonb_object_agg(risk_key, cnt) AS risk_stats
        FROM (
            SELECT
                COALESCE(a.risk_level, 'unknown') AS risk_key,
                COUNT(*)::INT                     AS cnt
            FROM public.user_assessments a
            WHERE (user_uuid IS NULL OR a.user_id = user_uuid)
              AND a.overall_score IS NOT NULL
    ...
    by_scale AS (
        SELECT jsonb_object_agg(scale_key, cnt) AS scale_stats
        FROM (
            SELECT
                COALESCE(a.scale, 'unknown') AS scale_key,
                COUNT(*)::INT                AS cnt
            FROM public.user_assessments a
            WHERE (user_uuid IS NULL OR a.user_id = user_uuid)
              AND a.overall_score IS NOT NULL
    ...
    by_tier AS (
        SELECT jsonb_object_agg(tier_key, cnt) AS tier_stats
        FROM (
            SELECT
                COALESCE(a.circular_economy_tier->>'tier', 'unknown') AS tier_key,
                COUNT(*)::INT                                          AS cnt
            FROM public.user_assessments a
            WHERE (user_uuid IS NULL OR a.user_id = user_uuid)
              AND a.overall_score IS NOT NULL
              AND a.circular_economy_tier IS NOT NULL
            GROUP BY a.circular_economy_tier->>'tier'
        ) sub
    )
    SELECT
        s.total, s.completed, s.avg_sc, s.median_sc, s.min_sc, s.max_sc,
        s.avg_conf, s.avg_tf, s.avg_ev, s.avg_cp, s.avg_pcs, s.avg_ras,
        COALESCE((SELECT ind_stats   FROM by_industry), '{}'::jsonb),
        COALESCE((SELECT risk_stats  FROM by_risk),     '{}'::jsonb),
        COALESCE((SELECT scale_stats FROM by_scale),    '{}'::jsonb),
        COALESCE((SELECT tier_stats  FROM by_tier),     '{}'::jsonb)
    FROM stats s;
END;
$$;
