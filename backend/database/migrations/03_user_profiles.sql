...
-- Drop the profiles table if it exists (cascades to indexes, policies, triggers)
DROP TABLE IF EXISTS profiles CASCADE;

-- Drop all functions defined in this migration (regardless of signature)
DO $$
DECLARE
...
-- Note: Modifications to the assessments table (dropping session_id, making user_id NOT NULL, adding foreign key) are handled idempotently within the main migration script, so no additional cleanup is required here.
