...
/**
 * Create handler functions for assessment actions
 * Used by pages that use AssessmentColumn
 *
 * @param {Object} params - Parameters for creating handlers
 * @param {Function} params.navigate - Navigation function
 * @param {Function} params.executeExport - Export execution function
 * @returns {Object} Handler functions object
 */
export const createAssessmentHandlers = ({ navigate, executeExport }) => {
  const handleReevaluate = (assessment) => {
    navigate('/', {
      state: {
        formData: {
  ...
  const handleDownloadPDF = async (assessment, scoringResult) => {
    if (!scoringResult) {
      // Toast will be imported and used directly in the calling component
      throw new Error('No result data available to export');
    }

    await executeExport(
      () =>
        import('@/features/export').then(({ exportAssessmentPDF }) =>
          exportAssessmentPDF(assessment, { elementId: `assessment-${assessment.id}` }),
        ),
      'PDF',
    );
  };
  ...
  const handleDownloadCSV = async (assessment, scoringResult) => {
    ...
    await executeExport(
      () =>
        import('@/features/export').then(({ exportAssessmentCSV }) =>
          exportAssessmentCSV(assessment),
        ),
      'CSV',
    );
  };
